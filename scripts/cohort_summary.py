...
Outputs:
  ...
  reports/cohort.csv    one row per grantee, the report's results table
...
from oku_wallet_cohort import GRANT_ID as OKU_GRANT_ID  # noqa: E402
...
CSV_COLUMNS = ["slug", "grantee", "grant_id", "scope", "ongoing", "op_budget", "start", "end",
...
def main() -> None:
    ...
    gs.sort(key=lambda g: -g["delta_tvl_usd"])
    ...
    print(f"wrote {a.out / 'cohort.json'} and {a.out / 'cohort.csv'}")
